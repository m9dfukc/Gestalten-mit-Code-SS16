  ...
  for (int i=0; i < rows; i++) {
    for (int j=0; j < cols; j++) {
      if (stripesB) {
        ...
        g[i][j] = (j%distProbability == 0 || i%(distProbability*6) == 0) ? 0 : 1; 
      }
...
void process() {
  ...
  for (int i=0; i < rows; i++) {
    for (int j=0; j < cols; j++) {
      ...
      else if ((current == 1) && (sum > 2) && (top == 0 && bottom == 1)) buffer[i][j] = 1;
      else if ((current == 1) && (sum > 2))                              buffer[i][j] = 0;        
      else if ((current == 0) && (sum == 3))                             buffer[i][j] = bottom;        
