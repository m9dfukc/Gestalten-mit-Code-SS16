...
void draw() {
  ...
  for (PVector point : points) {
    ...
    // color lookup
    int pIndex = (int)(palette.length*noise(index/50.0)); // vary noise function
    stroke(palette[pIndex], 20); // maybe add alpha
    ...
    float n = 15 * map(noise(point.x, point.y, time), 0, 1, -1, 1); // use factor of 10/100/1000/...
